...
def gotourl(url):
    ...
    try:
        ...
        print e
        accesslogfile=open('./log/ynfailedurl.log','a+')
        ...
        for y in range(0,256,3):
            ...
            if imgpixel != None:
                ...
                if (pixelr>184) & (pixelr<188) & (pixelg<3) & (pixelb<3):
                    ...
                    returnstr += '{0},{1},{2},{3},{4},{5}|'.format(tilex,tiley,x,y,onetime,status)
    if returnstr != '':
        url="http://127.0.0.1:8000/road/yngetgeo/?returnstr={0}".format(returnstr)
        gotourl(url)
...
def main(num):
    ...
    主函数
    """
    with open('./log/getynpic.log','wb+') as logfile:
        logfile.truncate()
    with open('./log/ynfailedurl.log','wb+') as logfile:
        logfile.truncate()
    otherpattern=re.compile(r'\n|\s',re.S)
    njfile=open('./ynalltiles','r')
    alllines=njfile.readlines()
    everyline=len(alllines)/num+1
    p = Pool(processes=num)
    for num in xrange(0,num):
        partlines=alllines[num*everyline:(num+1)*everyline]
        p.apply_async(oneprocess,args=(partlines,))
    p.close()
    p.join()
    njfile.close()
    with open('./log/getynpic.log','r') as logfile:
        for oneline in logfile.readlines():
            oneinfo=re.sub(otherpattern,'',oneline)
            if oneinfo != '':
                infolist=oneinfo.split(',')
                getimage(infolist[0],infolist[1])
    with open('./log/ynfailedurl.log','r') as logfile:
        for oneline in logfile.readlines():
